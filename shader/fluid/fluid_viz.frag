...
tgrad_t tex_grad2(vec2 uv)
{
	...
	float scale = 1.f/sqrt(size.x*size.y);
...
void main()
{
	...
	vec3 n = normalize(vec3(grad_x.x,grad_x.y,1.f/S));

	float f = 0.05 + 0.95*clamp(dot(n,sun),0,1);

	float speed = length(val.xy);

	float color_val = abs(val.r);
	...
	if (uv.x < 0.f || uv.y < 0.f || uv.x > 1.f || uv.y > 1.f) {
		...
		FragColor = vec4(f*color.rgb,0.8f);
	}
}
