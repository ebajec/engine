...
struct FLIPFluidSim
{
    ...
    size_t get_depost_buf_header_size() { return DIMS * sizeof( uint32_t ); }

    size_t get_depost_buf_data_size() { return ( ( 1 + grid_w ) * ( grid_h ) + ( grid_w ) * ( 1 + grid_h ) ) * sizeof( VelocityCell ); }
    ...
    int init( ev2::GfxContext* ctx, uint32_t w, uint32_t h )
    {
        ...
        ev2::ImageUsageFlags usage = ev2::IMAGE_USAGE_STORAGE_BIT | ev2::IMAGE_USAGE_SAMPLED_BIT;
        ...
        deposit_buf = ev2::create_buffer( ctx, deposit_buf_size, ev2::BUFFER_USAGE_STORAGE_BUFFER_BIT );
        ...
        size_t particle_bvh_size =
            align_up( sizeof( BVHHeader ) + sizeof( GpuAABB2f ) * bvh_size( particle_count, BVH_INTERNAL_SIZE ), 16 );
        ...
        ev2::bind_buffer( ctx, bindings, "Particles", particles, 0, particle_count * sizeof( FluidParticle ) );
    ...
    void step_sim( ev2::GfxContext* ctx, const SimParams& params )
    {
        ...
        struct alignas( sizeof( glm::vec2 ) )
        {
            ...
            glm::vec2 cursor2;
            uint32_t cursor_flags;
        } pc_particle = { .count        = particle_count,
                          .step         = (uint32_t)step,
        ...
        if( params.b_compute_bvh )
        {
            ...
            ev2::BufferID sort_input[] = { particles, particles_mirror };
            ...
            struct
            {
                glm::vec2 scale;
                              ...
                              glm::vec2( ( MORTON_CODE_MAX_DIM + grid_w - 1 ) / grid_w, ( MORTON_CODE_MAX_DIM + grid_h - 1 ) / grid_h ) };
            ...
            ev2::ComputePipelineID p_bvh_level =
                ev2::load_compute_pipeline( ctx, "fluid://shader/linear_bvh_2d_fluid.slang", "compute_level" );
...
int FluidApp::initialize( int argc, char** argv )
{
    ...
    main_panel.reset( new ImageViewerPanel( this, 200, 0, 500, 500, "core://pipeline/screen_quad.yaml", "Interactive Simulation" ) );
    right_panel.reset( new ImageViewerPanel( this, 700, 0, 500, 500, "fluid://pipeline/residuals.yaml", "Residuals" ) );
...
int FluidApp::update()
{
    ...
    bool is_panel_clicked = this->input.right_mouse_pressed && main_panel->panel->is_content_selected();
...
void FluidApp::render()
{
    ...
    glm::mat3x2 world = glm::mat3x2( glm::vec2( 2.f * aspect / (float)sim->grid_w, 0 ), glm::vec2( 0, 2.f / (float)sim->grid_h ),
                                     glm::vec2( -aspect, -1.f ) );

    // second pass renders with alpha blending
    ev2::GfxPassInfo pass_info = { .target      = main_panel->panel->get_target(),
    ...
    if( b_enable_flux_arrows )
    {
        ...
        struct alignas( 8 )
        {
            glm::ivec2 size;
            uint32_t v_img[2];
        } pc = { .size  = glm::ivec2( sim->grid_w, sim->grid_h ),
                 .v_img = {
        ...
        ev2::cmd_custom( pass,
                         [w = pc.size.x, h = pc.size.y]( VkCommandBuffer cmds )
                         {
    ...
    ev2::bind_buffer( ctx, particle_bindings, "Particles", sim->particles, 0, sim->particle_count * sizeof( FluidParticle ) );
    ...
    ev2::cmd_custom( pass, [this]( VkCommandBuffer cmds ) { vkCmdDraw( cmds, 6, sim->particle_count, 0, 0 ); } );
    ...
    if( params.b_compute_bvh )
    {
        ...
        ev2::cmd_custom(
            pass, [bvh_count]( VkCommandBuffer cmds )
